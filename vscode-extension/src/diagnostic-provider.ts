...
export class DiagnosticProvider {
  ...
  private findingsMap: Map<string, PyneatFinding[]> = new Map();
   ...
   */
  private debounceScan(doc: vscode.TextDocument, delay: number): void {
  ...
  private updateDiagnostics(
    ...
    const threshold = config.get('severityThreshold', 'info') as string;
   ...
   */
  public getFindings(uri: vscode.Uri): PyneatFinding[] {
   ...
   */
  private createDiagnostic(finding: PyneatFinding, doc: vscode.TextDocument): vscode.Diagnostic {
    ...
    diagnostic.relatedInformation = [
      new vscode.DiagnosticRelatedInformation(
        new vscode.Location(doc.uri, new vscode.Range(finding.start_line - 1, 0, finding.start_line - 1, 0)),
        `Severity: ${finding.severity.toUpperCase()} | CWE: ${finding.cwe_id || 'N/A'}`
      ),
      new vscode.DiagnosticRelatedInformation(
        new vscode.Location(doc.uri, new vscode.Range(finding.start_line - 1, 0, finding.start_line - 1, 0)),
        `Fix: ${finding.fix_constraints[0] || 'See documentation'}`
      )
    ];
   ...
   */
  private createRange(finding: PyneatFinding, doc: vscode.TextDocument): vscode.Range {
   ...
   * Build hover message
   */
  private buildHoverMessage(finding: PyneatFinding): string {
    const lines: string[] = [];
   ...
   */
  private updateContextKeys(issueCount: number): void {
   ...
   */
  private updateStatus(state: 'ready' | 'scanning' | 'error', message: string): void {
    switch (state) {
      case 'scanning':
   ...
   */
  async scanAllOpen(): Promise<void> {
