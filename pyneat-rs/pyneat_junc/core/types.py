...
class AgentMarker:
    ...
    suggested_fix: Optional[str] = None      # AI-suggested code fix
    attack_scenario: Optional[str] = None     # How an attacker would exploit this
    references: Tuple[str, ...] = field(default_factory=tuple)   # CWE/OWASP/CVE links
    fix_confidence: Optional[float] = None   # Confidence in suggested fix (0.0-1.0)
    alternative_fixes: Tuple[str, ...] = field(default_factory=tuple)  # Alternative fix options
    ...
    def __post_init__(self):
        ...
        for fname in ("related_markers", "fix_constraints", "do_not", "verify", "resources", "references", "alternative_fixes"):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AgentMarker":
        ...
        tuple_fields = (
            "related_markers", "fix_constraints", "do_not", "verify",
            "resources", "references", "alternative_fixes",
        )
    ...
    def to_yaml_comment(self, language: str = "python") -> str:
        """Serialize to a multi-line PYNAGENT YAML comment block.

        Generates language-aware comment syntax:
          Python:   # PYNAGENT: ...  (inline)  or  # ... (multiline)
          JS/TS:    // PYNAGENT: ...
          HTML:     <!-- PYNAGENT: ... -->
          Ruby:     # PYNAGENT: ...
          PHP:      // PYNAGENT: ...
        ...
        The YAML block is always indented under a # PYNAGENT: marker.
        """
        comment_prefix = self._get_comment_prefix(language)

        lines = [
            f"{comment_prefix} PYNAGENT:",
            f"{comment_prefix}   marker_id: {self.marker_id}",
            f"{comment_prefix}   issue_type: {self.issue_type}",
            f"{comment_prefix}   rule_id: {self.rule_id or 'unknown'}",
            f"{comment_prefix}   severity: {self.severity}",
            f"{comment_prefix}   line: {self.line}",
        ]
        if self.file_path:
            lines.append(f"{comment_prefix}   file: {self.file_path}")
        if self.why:
            why_text = self.why.replace('\n', ' ')[:120]
            lines.append(f"{comment_prefix}   why: {why_text}")
        if self.hint:
            hint_text = self.hint.replace('\n', ' ')[:120]
            lines.append(f"{comment_prefix}   hint: {hint_text}")
        if self.impact:
            lines.append(f"{comment_prefix}   impact: {self.impact}")
        if self.fix_constraints:
            for fc in self.fix_constraints[:3]:
                fc_text = fc.replace('\n', ' ')[:120]
                lines.append(f"{comment_prefix}   fix_constraints:")
                lines.append(f"{comment_prefix}     - {fc_text}")
        if self.do_not:
            lines.append(f"{comment_prefix}   do_not:")
            for dn in self.do_not[:3]:
                dn_text = dn.replace('\n', ' ')[:120]
                lines.append(f"{comment_prefix}     - {dn_text}")
        if self.verify:
            lines.append(f"{comment_prefix}   verify:")
            for v in self.verify[:3]:
                v_text = v.replace('\n', ' ')[:120]
                lines.append(f"{comment_prefix}     - {v_text}")
        if self.resources:
            lines.append(f"{comment_prefix}   resources:")
            for r in self.resources[:3]:
                r_text = r.replace('\n', ' ')[:120]
                lines.append(f"{comment_prefix}     - {r_text}")
        if self.cwe_id:
            lines.append(f"{comment_prefix}   cwe_id: {self.cwe_id}")
        if self.owasp_id:
            lines.append(f"{comment_prefix}   owasp_id: {self.owasp_id}")
        if self.cvss_score:
            lines.append(f"{comment_prefix}   cvss_score: {self.cvss_score}")
        lines.append(f"{comment_prefix}   confidence: {self.confidence}")
        lines.append(f"{comment_prefix}   can_auto_fix: {str(self.can_auto_fix).lower()}")
        if self.detected_at:
            lines.append(f"{comment_prefix}   detected_at: {self.detected_at}")
        return "\n".join(lines)

    def _get_comment_prefix(self, language: str) -> str:
        """Get the comment prefix for a given language."""
        if language in ("python", "ruby", "perl", "shell", "yaml"):
            return "#"
        elif language in ("javascript", "typescript", "java", "c", "cpp", "csharp", "go", "rust", "php", "swift", "kotlin"):
            return "//"
        elif language in ("html", "xml", "vue", "svelte"):
            return "<!-- -->".replace(" -->", "  -->").replace("<!-- ", "<!--").rstrip()
    ...
    def to_json_comment(self) -> str:
        """Serialize to a compact JSON PYNAGENT comment (legacy format)."""
        import json
        data = self.to_dict()
    ...
    def to_comment(self) -> str:
        """Legacy single-line PYNAGENT comment. Prefer to_yaml_comment()."""
        return self.to_json_comment()
