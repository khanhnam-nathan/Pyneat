...
# --------------------------------------------------------------------------
# LLM Analyzer Client (HTTP wrapper mirroring the Rust LlmAnalyzer)
# --------------------------------------------------------------------------
...
class LlmAnalysisResult:
    ...
    def __init__(
        ...
        self.alternative_fixes = alternative_fixes or []
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "LlmAnalysisResult":
        return cls(
            ...
            fix_confidence=data.get("fix_confidence"),
        )


class LlmAnalyzerClient:
    """HTTP-based LLM analyzer client.

    Mirrors the Rust LlmAnalyzer but implemented in pure Python so it works
    without requiring the pyneat-rs Rust extension.
    """
    ...
    def _find_api_key(self) -> Optional[str]:
        for var in ("PYNEAT_LLM_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY"):
    ...
    def _build_prompt(self, code: str, context: str) -> str:
        return (
            ...
            '  "alternative_fixes": ["alternative fix option 1", ...],\n'
    ...
    def analyze(self, code: str, context: str) -> LlmAnalysisResult:
        """Synchronous LLM analysis (runs in a thread pool internally)."""
        try:
        ...
        try:
            return loop.run_until_complete(self._analyze_async(code, context))
        except RuntimeError:
            return asyncio.run(self._analyze_async(code, context))
    ...
    async def _analyze_async(
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        raise LlmAnalysisError(
                            f"API error {response.status}: {body_text}"
                        )
        ...
        if trimmed.startswith("```json"):
            trimmed = trimmed[7:].rstrip("```").strip()
        elif trimmed.startswith("```"):
            trimmed = trimmed[3:].rstrip("```").strip()

        try:
            parsed = json.loads(trimmed)
        except json.JSONDecodeError as e:
            raise LlmAnalysisError(f"JSON parse error: {e} — body: {trimmed[:200]}")

        return LlmAnalysisResult.from_dict(parsed)

    async def analyze_async(
        self, code: str, context: str
    ) -> LlmAnalysisResult:
        """Async LLM analysis."""
        return await self._analyze_async(code, context)
...
class RateLimiter:
    ...
    def acquire(self, key: str) -> bool:
        """Return True if a slot is available, False if rate-limited."""
        now = time.monotonic()
    ...
    def wait_time(self, key: str) -> float:
        ...
        if len(bucket) < self.max_per_window:
            return 0.0
        return max(0.0, self.window_seconds - (time.monotonic() - bucket[0]))
...
class RuleEngine:
    ...
    def __init__(self, rules: List[Rule] = None, plugin_loader: Optional[PluginLoader] = None):
        ...
        # LLM enrichment
        self._llm_client: Optional[LlmAnalyzerClient] = None
        self._rate_limiter = RateLimiter(max_per_window=10, window_seconds=60.0)
        self._enrich_guardrails_patterns = {
            "suspicious_import": re.compile(
                r'__import__\s*\(["\']os["\']\s*\)|'
                r'__import__\s*\(["\']subprocess["\']\s*\)|'
            ...
            "filesystem_op": re.compile(
                r'\bopen\s*\(|'
                r'\bPath\s*\(.*\)\s*\.\s*(read_text|write_text|unlink|mkdir)|'
                r'\bos\s*\.\s*(remove|mkdir|rmdir|unlink|rename)|'
                r'\bshutil\s*\.(rmtree|copy|move)',
            ),
            "network_op": re.compile(
                r'\brequests\s*\.|'
                r'\burllib\s*\.|'
                r'\bhttp\s*\.|'
                r'\bsocket\s*\.|'
    ...
    def _get_llm_client(self) -> Optional[LlmAnalyzerClient]:
        """Lazily create the LLM client (checks for API key on first access)."""
        if self._llm_client is None:
            client = LlmAnalyzerClient()
            if client.available:
                self._llm_client = client
        return self._llm_client

    def _check_guardrails(self, snippet: str, sent_length: int) -> Tuple[bool, str]:
        """Check if a code snippet passes LLM guardrails.

        Returns (passed, reason) — reason is empty string if passed.
        """
        snippet_lower = snippet.lower()
        for name, pattern in self._enrich_guardrails_patterns.items():
            if pattern.search(snippet):
                return False, f"Blocked: suspicious pattern '{name}' in code snippet"
        if len(snippet) > 0 and abs(len(snippet) - sent_length) > len(snippet) * 0.5:
            return False, f"Length mismatch: sent {sent_length} chars but snippet is {len(snippet)}"
        return True, ""

    def enrich_with_llm(
        self, markers: List, max_per_file: int = 10
    ) -> List:
        """Enrich high-severity markers with LLM analysis.

        For each marker with severity "critical" or "high", this method:
        - Calls the LLM analyzer (if API key is available)
        - Enriches the marker with: suggested_fix, attack_scenario, references,
          fix_confidence

        Args:
            markers: List of AgentMarker objects to enrich.
            max_per_file: Maximum number of markers to enrich per file (rate limit).

        Returns:
            Enriched list of AgentMarker objects (unchanged if no API key or rate-limited).
        """
        client = self._get_llm_client()
        if not client:
            logger.debug("enrich_with_llm: no API key available, skipping enrichment")
            return markers

        if not self._rate_limiter.acquire("llm_enrich"):
            logger.debug("enrich_with_llm: rate limit reached, skipping enrichment")
            return markers
        ...
        for marker in markers:
            if count >= max_per_file:
                break
            if marker.severity not in ("critical", "high"):
                enriched.append(marker)
                continue

            snippet = marker.snippet or ""
            if not snippet.strip():
                enriched.append(marker)
                continue

            guard_ok, reason = self._check_guardrails(snippet, len(snippet))
            if not guard_ok:
                logger.warning("enrich_with_llm: guardrails failed for %s — %s", marker.marker_id, reason)
                enriched.append(marker)
                continue

            context = (
                f"file={marker.file_path or 'unknown'}, "
                f"line={marker.line}, "
                f"issue={marker.issue_type}, "
                f"rule={marker.rule_id}, "
                f"cwe={marker.cwe_id or 'unknown'}"
            )

            try:
                result = client.analyze(snippet, context)
                if result.suggested_fix:
                    object.__setattr__(marker, "suggested_fix", result.suggested_fix)
                if result.attack_scenario:
                    object.__setattr__(marker, "attack_scenario", result.attack_scenario)
                if result.references:
                    object.__setattr__(marker, "references", tuple(result.references))
                if result.fix_confidence is not None:
                    object.__setattr__(marker, "fix_confidence", result.fix_confidence)
                if result.alternative_fixes:
                    object.__setattr__(marker, "alternative_fixes", tuple(result.alternative_fixes))
                if result.cwe_ids:
                    existing_cwe = marker.cwe_id or ""
                    combined_cwe = ",".join(
                        filter(None, [existing_cwe] + result.cwe_ids[:3])
                    )
                    object.__setattr__(marker, "cwe_id", combined_cwe)
                count += 1
                logger.debug(
                    "enrich_with_llm: enriched %s with LLM result (confidence=%.2f)",
                    marker.marker_id, result.confidence,
                )
            except LlmAnalysisError as e:
                logger.warning("enrich_with_llm: LLM analysis failed for %s — %s", marker.marker_id, e)
            except Exception as e:
                logger.warning("enrich_with_llm: unexpected error for %s — %s", marker.marker_id, e)

            enriched.append(marker)

        return enriched

    async def _enrich_marker_async(
        self, marker, client: LlmAnalyzerClient
    ) -> Tuple[int, Any, Optional[str]]:
        """Async helper: enrich a single marker and return (idx, marker, error)."""
        snippet = marker.snippet or ""
        if not snippet.strip():
            return marker.line, marker, None

        guard_ok, reason = self._check_guardrails(snippet, len(snippet))
        if not guard_ok:
            return marker.line, marker, reason
        ...
        try:
            result = await client.analyze_async(snippet, context)
            if result.suggested_fix:
                object.__setattr__(marker, "suggested_fix", result.suggested_fix)
            if result.attack_scenario:
                object.__setattr__(marker, "attack_scenario", result.attack_scenario)
            ...
            return marker.line, marker, str(e)
        except Exception as e:
            return marker.line, marker, str(e)

    async def async_enrich_markers_batch(
        self, markers: List, max_concurrent: int = 5
    ) -> List:
        """Asynchronously enrich markers with LLM analysis using bounded concurrency.
        ...
        Returns:
            Enriched list of AgentMarker objects.
        """
        client = self._get_llm_client()
        if not client:
            logger.debug("async_enrich_markers_batch: no API key, skipping")
            return markers

        high_severity = [
            (i, m) for i, m in enumerate(markers)
            if m.severity in ("critical", "high") and (m.snippet or "").strip()
        ]
        if not high_severity:
            return markers

        enriched_map: Dict[int, Any] = {i: m for i, m in enumerate(markers)}
        semaphore = asyncio.Semaphore(max_concurrent)

        async def sem_enrich(idx: int, marker):
            async with semaphore:
                if not self._rate_limiter.acquire("llm_batch"):
                    return idx, marker, "rate_limited"
                _, enriched, err = await self._enrich_marker_async(marker, client)
                return idx, enriched, err

        tasks = [sem_enrich(i, m) for i, m in high_severity]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for res in results:
            if isinstance(res, Exception):
                continue
            idx, enriched, err = res
            if err:
                logger.debug("async_enrich_markers_batch: idx=%d err=%s", idx, err)
            enriched_map[idx] = enriched

        return [enriched_map[i] for i in range(len(markers))]
    ...
    def process_file(self, file_path: Path, check_conflicts: bool = False, language: str = "auto", enrich: bool = False) -> TransformationResult:
        ...
        Args:
            ...
            enrich: If True, enrich critical/high markers with LLM analysis.
        """
        try:
            ...
            result = self.process_code_file(code_file, check_conflicts=check_conflicts, enrich=enrich)
    ...
    def process_code_file(
        ...
        for rule in sorted_rules:
            ...
            if result.success:
                ...
                # Aggregate AgentMarkers from ALL rules (Phase 1.1 — killing feature)
                if hasattr(result, 'agent_markers') and result.agent_markers:
                    all_agent_markers.extend(result.agent_markers)
        ...
        # Convert security findings to AgentMarkers
        security_markers = [
            security_finding_to_marker(f, language=cf.language, file_path=str(cf.path))
            for f in all_security_findings
        ]

        # Merge: security-finding-based markers + direct agent_markers from all rules
        merged_markers = security_markers + all_agent_markers

        # LLM enrichment (if requested and API key is available)
        if enrich and merged_markers:
            merged_markers = self.enrich_with_llm(merged_markers, max_per_file=10)
