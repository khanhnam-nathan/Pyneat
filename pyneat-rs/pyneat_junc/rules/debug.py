        ...
        try:
            ...
            for change in transformer.changes_made:
                marker = self.build_agent_marker(
                    change=change,
                    code_file=code_file,
                    issue_type="debug-artifact",
                    severity="low",
                    hint="Remove debug artifacts: print statements, pdb breakpoints, debug comments",
                    why=change,
                    confidence=0.95,
                    category="quality",
                )
        ...
        if len(body) == 1:
            ...
            if isinstance(stmt, cst.Expr):
                if isinstance(stmt.value, cst.Call):
                    func = stmt.value.func
                    name = self._get_call_name(func)
                    if name in ('pdb.set_trace', 'ipdb.set_trace', 'pudb.set_trace', 'set_trace', 'breakpoint'):
                        self.removed_pdb += 1
                        return cst.RemovalSentinel.REMOVE
