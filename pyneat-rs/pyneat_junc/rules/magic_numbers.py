    ...
    def apply(self, code_file: CodeFile) -> TransformationResult:
        try:
            ...
            if count:
                ...
                # Build markers for each flagged magic number
                for i in range(count):
                    marker = self.build_agent_marker(
                        change=f"Flagged magic number at line",
                        code_file=code_file,
                        issue_type="magic-number",
                        severity="medium",
                        hint="Replace magic numbers with named constants for better readability",
                        why=f"Magic number detected",
                        confidence=0.85,
                        category="quality",
                    )
