...
class UnusedImportRule(Rule):
    ...
    def apply(self, code_file: CodeFile) -> TransformationResult:
        try:
            ...
            # Build markers for each removed import
            for name in removed_names:
                marker = self.build_agent_marker(
                    change=f"Removed unused import: {name}",
                    code_file=code_file,
                    issue_type="unused-import",
                    severity="low",
                    hint="Remove unused import statements to clean up the code",
                    why=f"Unused import: {name}",
                    confidence=0.95,
                    category="import",
                )
