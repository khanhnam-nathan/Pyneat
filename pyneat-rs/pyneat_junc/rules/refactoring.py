        ...
        try:
            ...
            if self._has_arrow_anti_pattern(content):
                ...
                markers.append(self.build_agent_marker(
                    change="Refactored arrow anti-pattern (nested if-else)",
                    code_file=code_file,
                    issue_type="deep-nesting",
                    severity="medium",
                    hint="Refactor deeply nested if-else chains into early returns or a dictionary dispatch pattern",
                    why="Deeply nested if-else chains (arrow anti-pattern) reduce code readability and maintainability",
                    confidence=0.9,
                    category="refactor",
                ))
            ...
            for ch in except_changes:
                ...
                markers.append(self.build_agent_marker(
                    ...
                    severity="medium",
                    hint="Add specific exception handling or re-raise with context",
            ...
            for ch in eval_changes:
                changes.append(ch)
                markers.append(self.build_agent_marker(
                    change=ch,
                    code_file=code_file,
                    issue_type="dangerous-eval",
                    severity="high",
                    hint="Replace eval/exec with safer alternatives like ast.literal_eval or JSON parsing",
                    why="eval() and exec() can execute arbitrary code, creating RCE risks",
                    confidence=0.95,
                    category="security",
                ))
