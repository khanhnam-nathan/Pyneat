...
class DataclassSuggestionRule(Rule):
    ...
    def apply(self, code_file: CodeFile) -> TransformationResult:
        try:
            ...
            # Build markers for each candidate
            for class_name, line_no, reason in candidates:
                change = f"Suggest @dataclass: {class_name} at line {line_no}"
                marker = self.build_agent_marker(
                    change=change,
                    code_file=code_file,
                    issue_type="dataclass-suggestion",
                    severity="low",
                    hint="Consider using @dataclass decorator for cleaner data class definition",
                    why=change,
                    confidence=0.85,
                    category="quality",
                )
