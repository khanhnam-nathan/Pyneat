        ...
        try:
            ...
            if hasattr(code_file, 'cst_tree') and code_file.cst_tree is not None:
                ...
                try:
                    ...
                    return self._create_result(code_file, content, [], markers)

            transformer = _IsNotNoneTransformer()
            new_tree = tree.visit(transformer)
            new_content = new_tree.code

            # Build markers for each change
            for change in transformer.changes:
                marker = self.build_agent_marker(
                    change=change,
                    code_file=code_file,
                    issue_type="comparison-style",
                    severity="medium",
                    hint="Use 'is not None' instead of '!= None' for PEP8 compliance",
                    why=change,
                    confidence=0.95,
                    category="quality",
                )
            ...
            return self._create_result(code_file, new_content, transformer.changes, markers)
