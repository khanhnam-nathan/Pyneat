...
class ImportCleaningRule(Rule):
    ...
    def apply(self, code_file: CodeFile) -> TransformationResult:
        try:
            ...
            changes: List[str] = []
            markers: List = []
            num_imports = len(transformer._imports)
            change = f"Collected {num_imports} import statement(s)"
            changes.append(change)
            markers.append(self.build_agent_marker(
                change=change, code_file=code_file,
                issue_type="import-cleanup",
                severity="low",
                hint="Imports have been deduplicated and organized below the module docstring",
                why="Import deduplication reduces cognitive load and ensures consistent module structure",
                confidence=1.0,
                can_auto_fix=True,
                category="import",
            ))
