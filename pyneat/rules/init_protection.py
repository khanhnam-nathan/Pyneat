...
from pyneat.core.types import AgentMarker, CodeFile, RuleConfig, TransformationResult
...
class InitFileProtectionRule(Rule):
    ...
    def apply(self, code_file: CodeFile) -> TransformationResult:
        try:
            ...
            # Build markers for each protected import
            for name in protected:
                marker = self.build_agent_marker(
                    change=f"Protected __init__ re-export: {name}",
                    code_file=code_file,
                    issue_type="init-protection",
                    severity="high",
                    hint="This import is protected because it supports __all__ re-export",
                    why=f"Protected __init__ re-export: {name}",
                    confidence=0.95,
                    category="quality",
                )
                markers.append(marker)

            return self._create_result(code_file, new_content, changes, markers)
