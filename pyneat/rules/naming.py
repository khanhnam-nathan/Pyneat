    ...
    def apply(
        ...
        try:
            ...
            for old, new in rename_map.items():
                if old != new:
                    change = f"Renamed class: '{old}' -> '{new}'"
                    changes.append(change)
                    markers.append(self.build_agent_marker(
                        change=change, code_file=code_file,
                        issue_type="naming-convention",
                        severity="low",
                        hint=f"Rename class from snake_case '{old}' to PascalCase '{new}' for PEP8 compliance",
                        why="Class names should use PascalCase per PEP8 naming conventions",
                        confidence=0.95,
                        can_auto_fix=True,
                        category="naming",
                    ))
